...
- **`internal/`** - All core application logic (non-public Go packages).
  ...
  - **`internal/cmd/`** - Cobra command implementations for all CLI commands (init, tutorial, create, fork, list, delete, pause, resume, connect, psql, sql, schema, logs, password, pricing, rename, status, id, feedback, api-key, login, logout, config, mcp, serve, version, upgrade, completion, payment, space, member, invite). Each command lives in its own file, named to match the command in snake_case (e.g. `ghost payment list` → `payment_list.go`). Helper files like `completion.go` and `errors.go` contain shared utilities. Commands that are not yet ready for public release can be gated behind the `GHOST_EXPERIMENTAL` env var (see `internal/common/app.go`'s `App.Experimental` field).
  - **`internal/api/`** - API client layer. Includes an OpenAPI-generated REST client (`client.go`, `types.go`), a generated strict HTTP server (`server.go`, implemented by `internal/server`), shared HTTP client singleton, and request/response types. **Do not edit `client.go`, `types.go` or `server.go` by hand** — they are generated from `openapi.yaml` (see [Code Generation](#code-generation)). The `mock/` subdirectory contains a generated mock of `ClientWithResponsesInterface` for use in tests.
  - **`internal/server/`** - A self-hosted implementation of the Ghost API for a single Postgres cluster, built for personal use now that the hosted service is winding down. Implements `api.StrictServerInterface`: a Ghost database is a Postgres database named after its ID (reported via the `dbname` field, which the CLI's `common.DatabaseName` reads), a fork is `CREATE DATABASE ... TEMPLATE`, pause is `ALLOW_CONNECTIONS false`, and the one `tsdbadmin` role owns every database with one shared password. Bookkeeping lives in the maintenance database's `ghost` schema. One space, one bearer token (`GHOST_SERVER_API_KEY`), no billing: seventeen operations do real work, the rest (`unsupported.go`) answer 501. `server_test.go` needs a throwaway TLS-enabled cluster via `GHOST_SERVER_TEST_POSTGRES_URL` and skips otherwise. `Dockerfile.server` builds the image.
...
**Read-only enforcement.** Visualized queries run in-process through the serve handler, whose DSN honors the `read_only` config option (the immutable read-only connection GUC), so MCP-driven visualizations can't bypass read-only mode. That GUC exists only on Timescale Cloud; against `ghost-server` (stock Postgres) `read_only` gates MCP tool registration but does not stop writes at the database. `/api/bootstrap` reports `readOnly` so the UI can surface a read-only indicator.
...
Every Ghost database can define custom MCP tools by marking Postgres functions with an `@mcp` comment — the first line of the `COMMENT ON FUNCTION` text is `@mcp`, and the remaining lines become the tool's description. Because the definition lives in the database itself, tools are shared across the space and travel with forks and shares.
...
The `slog` logger (constructed via `log.New` in `internal/log`) is reserved for long-running backend processes — the MCP server (`ghost mcp`) and the local web server (`ghost serve`) — since those are the only contexts where the CLI needs structured logging. Normal CLI commands should never emit log output — it looks bad and interferes with the user-facing output.
...
- `tea.WithoutSignalHandler()` — opt out of BubbleTea's default SIGINT/SIGTERM handler. Ctrl+C is already handled as a key press (above), and out-of-band signals are caught by the handler installed in `main.go`, which cancels the command context. Letting BubbleTea also install a handler creates a double-handler race for the same signals.
- `tea.WithContext(cmd.Context())` — wires the parent context into BubbleTea so external cancellation (e.g. SIGTERM caught by `main.go`) causes the program to exit. When the context is canceled, `program.Run()` returns an error wrapping `tea.ErrProgramKilled` and the underlying context error. **Exception:** if your model already has its own goroutine watching `ctx.Done()` and reporting cancellation back as a `tea.Msg` (e.g. `internal/common/wait.go`), passing `WithContext` is redundant and racy — pick one mechanism.
...
- **Registration**: `mcp.AddTool(s.mcpServer, newXxxTool(), s.handleXxx)` in `server.go`'s `registerTools()` method. **Exception:** read-only-sensitive (write) tools are registered in `registerReadOnlyTools()` instead — see the Read-only guard bullet below.
- **Read-only guard**: A tool that mutates or destroys existing resources is disallowed in read-only mode and must be wired up for it in two places: (1) register it in `registerReadOnlyTools()`/`removeReadOnlyTools()`, rather than directly in `registerTools()`; and (2) call `checkReadOnly(cfg)` in the handler after loading config, as defense-in-depth. The registration mechanism means these tools are *not registered at all* while read-only mode is on, and are added/removed dynamically (with a `tools/list_changed` notification) as the `read_only` config changes — see `reconcileReadOnlyTools()`. The in-handler `checkReadOnly` guard still matters: a client might call a tool in the window before it is removed, or ignore the notification. When adding or removing a read-only-sensitive tool, keep `registerReadOnlyTools()` and `removeReadOnlyTools()` in sync.

  Read-only mode applies only to tools that mutate or destroy *existing* resources (`ghost_delete`, `ghost_rename`, `ghost_pause`, `ghost_share`, `ghost_share_revoke`, `ghost_password`, `ghost_api_key_create`, `ghost_api_key_delete`). It deliberately still allows creating new resources — `ghost_create`, `ghost_fork`, `ghost_resume`, and the dedicated variants are *not* gated (creating/forking databases for experimentation is low-risk). So the read-only tool set does **not** align 1:1 with tools whose `ReadOnlyHint` annotation is `false`: several non-read-only-hinted tools remain available in read-only mode. Generated function tools (`internal/mcp/function`) are also out of scope — they intentionally ignore `read_only`.
